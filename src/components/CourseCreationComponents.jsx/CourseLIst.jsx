  ...
  <Box>
    ...
    <Typography variant="body2" color="text.secondary">
      {!myCourses || myCourses.length === 0
        ...
        : `You have enrolled in ${myCourses?.length} course${myCourses?.length !== 1 ? 's' : ''}`}
    </Typography>
  </Box>
    ...
    >
      ...
      </Grid> */}
      {
        filteredCoursesCompleted && filteredCoursesCompleted.length > 0 ? (
          <Gird container spacing={3} sx={{ mb: 4 }}>
            {filteredCoursesCompleted.map((course) => (
              <Gird item key={course._id || course.id} xs={12} sm={6} md={4} lg={4} xl={3}>
               <CourseCard course={course}/>
                </Gird>
            ))}
          </Gird>
